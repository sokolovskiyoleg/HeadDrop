...
public class EntityDeath implements Listener {
    ...
    private static final Enchantment LOOTING_ENCHANTMENT = Enchantment.LOOTING;
    ...
    public void populateEntityActions() {
        ...
        try {
            entityActions.put(EntityType.RABBIT, event -> handleEntityDrop(event, "RABBIT",
                    () -> {
                        ...
                        if ("Toast".equals(rabbit.getName())) {
        ...
        try {
            entityActions.put(EntityType.IRON_GOLEM, event -> handleEntityDrop(event, "IRON_GOLEM",
                    () -> {
                        ...
                        var attr = golem.getAttribute(org.bukkit.attribute.Attribute.MAX_HEALTH);
                        double maxHealth = attr != null ? attr.getValue() : 100.0;
                        double healthPercent = golem.getHealth() / maxHealth;
        ...
        try {
            entityActions.put(EntityType.SLIME,
                    event -> handleEntityDrop(event, "SLIME", EntityHead.SLIME::getSkull));
        } catch (NoSuchFieldError | IllegalArgumentException ignored) {
        }
        try {
        ...
        try {
            entityActions.put(EntityType.WITHER, event -> handleEntityDrop(event, "WITHER",
                    () -> {
                        ...
                        var attr = wither.getAttribute(org.bukkit.attribute.Attribute.MAX_HEALTH);
                        double maxHealth = attr != null ? attr.getValue() : 300.0;
                        boolean armored = wither.getHealth() > maxHealth / 2.0;
                        boolean invulnerable = wither.getInvulnerableTicks() > 0;
        ...
        try {
            entityActions.put(EntityType.HAPPY_GHAST, event -> handleEntityDrop(event, "HAPPY_GHAST",
                    () -> {
                        ...
                        } else if (event.getEntity() instanceof org.bukkit.entity.Zombie zombie && !zombie.isAdult()) {
                            return EntityHead.HAPPY_GHAST_BABY.getSkull();
                        }
